...
三条正式视频开始前，Agent 必须先录一条 30–45 秒样片，交给产品负责人确认。样片使用视频 02 的第一轮：新建对话、输入一次真实会计问题、等待回答完成，并展示答案顶部和一次缓慢滚动。

产品负责人明确回复“样片通过”之前，Agent 不得继续录视频 01、02、03 的正式版本。若样片需要调整，只重录样片；光标、输入、等待、滚动、画面范围和停留节奏全部确认后，再按同一标准录完三条正式视频。

## 文件结构

- `00-录制前检查.md`：账号、浏览器、隐私与画面设置。
- `00A-样片录制与审批.md`：正式录制前必须先通过的 30–45 秒样片。
...
Agent 不应边录边临场试错。先按《00A-样片录制与审批》交付一条样片并等待明确通过；之后每条正式视频再按《Agent 录制执行手册》完成无录屏彩排、重置和一次录成。出现错误直接停录并重新开始。
