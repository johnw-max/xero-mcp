...
| `xero_get_invoice`, `xero_get_supplier_bill`, `xero_get_credit_note`, `xero_get_bank_transaction`, `xero_get_manual_journal`, `xero_get_quote`, `xero_get_purchase_order` | `ledger.object.read_exact` | Exact returned accounting object | Exact provider object ID |
...
The 0.4.0-rc.1 local release contract exposes exactly 30 public tools. The only mutation entry points are the three typed Accounting Case tools above; legacy object-specific prepare/create tools and browser approval are not public. All actual provider creates remain behind the embedded ledger-control kernel and a one-shot permit.
