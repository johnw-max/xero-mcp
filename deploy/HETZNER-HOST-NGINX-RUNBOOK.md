...
- 0.3.0 固定公开 43 个工具：23 个读取、10 个准备、10 个受控执行。读取覆盖 Organisation、Contact、Account、Tax、Invoice/Bill、Credit Note、Payment、Quote、Purchase Order、Manual Journal、Item、Bank Transaction 和有界 Trial Balance。
...
本候选版本固定放在 `/opt/xero-accounting-mcp-demo-0.3.0-20260808.4`；以下命令均从该目录执行。从 `deploy/env.vps.example` 复制为只在 VPS 存在的 `deploy/.env.vps`，权限设为 `0600`。真实 Secret 不进入 Git、聊天、截图或模型上下文。
...
docker image inspect \
  xero-accounting-mcp-demo:0.3.0-xero-pilot-20260808.4 \
...
printf '%s' "$GREEN_HEALTH" | grep -F '"toolCount":43'
printf '%s' "$GREEN_HEALTH" | grep -F \
  '"toolsetHash":"a76bf853dc4bc71bf33e5b42f936fbcc9d6593d67d23e40dedccc4d1e2ae5d65"'
...
工具只允许把 `xero_accounting_mcp_demo` 在 18002/18004 之间切换；它先验证目标 health/ready。green 必须严格返回 ready，并精确匹配 0.3.0、43 个工具和工具集指纹。migration 020 成功后的常规 blue 回滚只允许目标 `18002`，其 health 必须为 HTTP 200、`status=ok`、`version=0.2.13`，ready 必须为 HTTP 200 的精确 `{"status":"ready","version":"0.2.13"}`。
...
- health/ready 200；Xero `/healthz` 必须精确包含 `version=0.3.0`、`toolCount=43` 和 `toolsetHash=a76bf853dc4bc71bf33e5b42f936fbcc9d6593d67d23e40dedccc4d1e2ae5d65`；Agent2 连接后的 `tools/list` 必须恰好返回同一 43 个工具；
...
```sh
sudo /usr/local/sbin/xero-agent2-uat-write-gate-030-20260808-4 close
sudo /usr/local/sbin/xero-agent2-uat-write-gate-030-20260808-4 status
```
...
| Hetzner green 部署通过 | “0.3.0 已在 18004 通过精确版本、43-tool contract、ready 与依赖连续性检查，尚未切流” |
