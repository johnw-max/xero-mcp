...

  {
    server.registerTool(
      "xero_prepare_accounting_case",
      {
        ...
        description: "Accepts ordinary source-document fields and compiles them into one tenant-bound, versioned Accounting Case. The server deterministically derives internal fact, lineage, event, source-unit and line IDs. Supply source-declared amounts only; Xero IDs, account codes, tax types, routes, computed ledger payloads and target identity remain server-side. First creation uses expected_version 0. This step performs no Xero write and always reports ledger_write as NOT_WRITTEN. Submitted/model-extracted facts remain source_truth_claim=NOT_VERIFIED and original_file_verified=false.",
