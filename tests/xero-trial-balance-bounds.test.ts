...
async function invokeTrialBalanceOverMcp(providerReport: Record<string, unknown>) {
  const repository = new InMemoryAccountingRepository();
  const provider = {
    getTrialBalance: vi.fn().mockResolvedValue(providerReport),
  ...
  try {
    ...
    return await client.callTool({ name: "xero_get_trial_balance", arguments: { date: "2026-08-06" } });
  } finally {
...
describe("Xero Trial Balance real MCP transport", () => {
  it.each([
    ...
    const callToolResult = await invokeTrialBalanceOverMcp(providerReport as unknown as Record<string, unknown>);
  ...
  it("bounds a long future Xero field name through the complete MCP CallToolResult", async () => {
    const providerReport = xeroReport(3);
    const report = providerReport.reports?.[0] as unknown as Record<string, unknown>;
    const longKey = `future-${"科\\\"📒".repeat(40_000)}`;
