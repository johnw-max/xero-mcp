...
describe("local deployment-equivalent Agent harness", () => {
  ...
  it("recomputes bundle and tool contract hashes and rejects drift", async () => {
    ...
    try {
      ...
      expect(toolContract.tool_count).toBe(28);
      expect(toolContract.tools).toHaveLength(28);
  ...
  it("keeps 28 backend tools but exposes only the typed Case profile to Luna", async () => {
    const contract = await buildToolContractEvidence(repoRoot);
    expect(contract.tool_count).toBe(28);
