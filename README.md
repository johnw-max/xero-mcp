...
| 层级 | 准确状态 |
|---|---|
| 当前线上受控 Demo | `xero-accounting-mcp-demo:0.3.0-xero-pilot-20260809.4`；43 个固定工具：23 个读取、10 个准备、10 个受控执行 |
...
| 本地验证 | typecheck/build 通过；当前完整默认回归 793 PASS、44 条条件跳过；另有历史强制门槛证据：HTTP/OAuth 2/2、fresh PostgreSQL 35/35 |
| 当前写闸 | 正式 Demo 默认关闭；当前 Work 账套尚未完成生产写入验收 |

因此当前结论是：`正式 Work 只读核心流程已通过 / 可用于受控 Demo / 多人生产写入未批准`。
...
> 仓库中保留少量 QuickBooks 模块、迁移和部署检查，是因为当前 VPS 与 PostgreSQL 仍采用共享运行底座，Xero 切换脚本必须保证既有 QuickBooks 服务不被误重启或破坏。这些内容不是本次 Xero 产品交付范围；后续应由开发团队拆分为独立服务或共享基础库。
...
旧线上记录、旧 15 工具验收和旧单张 Bill Demo 只可作为历史基线，不能证明本地 43 工具候选已经上线。
